defmodule Filesync.Cloud.DropboxTest do
  ...
  test "copy a file" do
    ...
    use_cassette "cloud/file_ops/copy_file" do
      {:ok, copy_meta} = Filesync.Cloud.file_copy(@dbx, from_path, to_path)
  ...
  test "move a file" do
    ...
    use_cassette "cloud/file_ops/move_file" do
      {:ok, move_meta} = Filesync.Cloud.file_move(@dbx, from_path, to_path)
  ...
  test "get_metadata from dropbox folder that exists" do
    use_cassette "cloud/get_metadata/folder" do
      {:ok, meta} = Filesync.Cloud.get_metadata(@dbx, @test_dir)
  ...
  test "get_metadata from dropbox item that doesn't exists" do
    use_cassette "cloud/get_metadata/not_found" do
      {:error, {reason, _}} = Filesync.Cloud.get_metadata(@dbx, "/fake")
  ...
  test "lists the root directory" do
    use_cassette "cloud/list_folder/root_dir" do
      {status, _} = Filesync.Cloud.list_folder(@dbx, "/")
      assert status == :ok
    end
  end
  ...
  test "list_folder: empty folder" do
    use_cassette "cloud/list_folder/no_files" do
      {:error, {reason, _}} = Filesync.Cloud.list_folder(@dbx, @test_dir)
  ...
  test "list_folder: path not found" do
    use_cassette "cloud/list_folder/not_found" do
      {:error, {reason, _}} = Filesync.Cloud.list_folder(@dbx, "/fake")
