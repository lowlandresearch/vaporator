defmodule Vaporator.Cloud.Dropbox do
  use Timex
  require Logger
  ...
  - dbx (Vaporator.Cloud.Dropbox): Dropbox client
  ...
  - Decodes and returns body binary content and headers (as
    Vaporator.Cloud.FileContent) if status code is 200
  ...
  *assumes* that the given path exists and can be streamed.
  ...
  Args:
  - fs (Vaporator.Cloud impl): Cloud file system
  ...
  Returns:
    {:ok, Vaporator.Cloud.FileContent}
      or
...
defimpl Vaporator.Cloud, for: Vaporator.Cloud.Dropbox do
  require Logger

  def get_hash!(_dbx, local_path) do
    Vaporator.Cloud.Dropbox.dbx_hash!(local_path)
  end

  def get_path(_dbx, local_root, local_path, dbx_root) do
    Vaporator.Cloud.Dropbox.get_dbx_path(local_root, local_path, dbx_root)
  end

  def list_folder(dbx, path, args \\ %{}) do
    Vaporator.Cloud.Dropbox.list_folder(dbx, path, args)
  end

  def get_metadata(dbx, path, args \\ %{}) do
    Vaporator.Cloud.Dropbox.get_metadata(dbx, path, args)
  end

  def file_download(dbx, path, dbx_api_args \\ %{}) do
    Vaporator.Cloud.Dropbox.file_download(dbx, path, dbx_api_args)
  end

  def file_upload(dbx, local_path, dbx_path, args \\ %{}) do
    Vaporator.Cloud.Dropbox.file_upload(dbx, local_path, dbx_path, args)
  end

  def file_update(dbx, local_path, dbx_path, args \\ %{}) do
    Vaporator.Cloud.Dropbox.file_update(dbx, local_path, dbx_path, args)
  end

  def file_remove(dbx, path, args \\ %{}) do
    Vaporator.Cloud.Dropbox.file_remove(dbx, path, args)
  end

  def folder_remove(dbx, path, args \\ %{}), do: file_remove(dbx, path, args)

  def file_copy(dbx, from_path, to_path, args \\ %{}) do
    Vaporator.Cloud.Dropbox.file_copy(dbx, from_path, to_path, args)
  end

  def file_move(dbx, from_path, to_path, args \\ %{}) do
    Vaporator.Cloud.Dropbox.file_move(dbx, from_path, to_path, args)
  end

  def update_setting(dbx, key, value) do
    Vaporator.Cloud.Dropbox.update_setting(dbx, key, value)
  end
end
